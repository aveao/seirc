...
class StackHandler(object):
    ...
    def dispatch_stack(self, msg):
        ...
        try:
            ...
            self.to_irc(
                ":SEIRC NOTICE %s :Error processing message from Stack: %s",
                self.nick,
                str(e),
            )
    ...
    def _send_lines(self, src, dst, lines):
        for line in lines.split("\n"):
            ...
            if (
                line.startswith("*")
                and line.endswith("*")
                or line.startswith("\x1F")
                and line.endswith("\x1F")
            ):
                line = "\x01ACTION " + line[1:-1] + "\x01"
            self.to_irc(":%s PRIVMSG %s :%s", src, dst, line)
    ...
    def stack_messageposted(self, msg):
        ...
        if msg.parent_message_id and msg.show_parent:
            ...
            if self._stack_show_reply(msg):
                return
        self._send_lines(
            tonick(msg.user.name), tochannel(msg.room.name), toplaintext(msg.content)
        )
        self._msg_cache[msg.data["message_id"]] = msg
    ...
    def stack_userentered(self, msg):
        if msg.user == self.stack.get_me():
            return
        self.to_irc(":%s JOIN %s", tonick(msg.user.name), tochannel(msg.room.name))

    def stack_userleft(self, msg):
        self.to_irc(":%s PART %s", tonick(msg.user.name), tochannel(msg.room.name))
